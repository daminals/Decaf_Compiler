...
class AST:  
  ...
  def create_record(self, ast):
      ...
      output += self.create_field_record(ast["body"]["fields"])

      # check constructors
      if "constructors" not in ast["body"]:
          raise Exception("Could not find constructors")
      output += self.create_constructor_record(ast["body"]["constructors"])

      # check methods
      if "methods" not in ast["body"]:
          raise Exception("Could not find methods")
      output += self.create_method_record(ast["body"]["methods"])
      return output
  ...
  """
  def create_field_record(self, ast_fields):
      output = "Fields:\n"
      for field_id in ast_fields.keys():
          field = ast_fields[field_id]
          # name
          if "ids" not in field:
  ...
  """
  def create_constructor_record(self, ast_constructors):
      output = "Constructors:\n"
      for constructor_id in ast_constructors.keys():
          constructor = ast_constructors[constructor_id]

          # modifiers
          if "modifiers" not in constructor:
              raise Exception("Could not find field modifiers")
          constructor_modifiers = self.create_modifiers_list_PRIVATE_PUBLIC(constructor["modifiers"])
  ...
  def create_method_record(self, ast_method):
      output = "Methods:\n"
      for method_id in ast_method.keys():
        method = ast_method[method_id]
        # name
